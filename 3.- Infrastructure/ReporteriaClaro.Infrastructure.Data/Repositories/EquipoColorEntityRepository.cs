...
{
    public partial class EquipoColorEntityRepository
    {
	    public async Task<EquipoColorEntity> FindAsync(int id)
	    {
		    IQueryable<EquipoColorEntity> consultaFiltrada = this.objectSet.Where(e => e.Activo && e.Id == id);
		    return await consultaFiltrada.SingleOrDefaultAsync();
		}

	    public async Task<IEnumerable<EquipoColorEntity>> GetAsync()
	    {
		    IQueryable<EquipoColorEntity> consultaFiltrada = this.objectSet.Where(e => e.Activo);
		    return await consultaFiltrada.OrderBy(e => e.Id).ToListAsync();
	    }

		public async Task<PagedResult<EquipoColorEntity>> GetPaginadoAsync(bool activeRecords, PagerInfo pagerInfo, SortingInfo sortingInfo)
	    {
		    IQueryable<EquipoColorEntity> consultaFiltrada = this.objectSet.Where(e => e.Activo == activeRecords);

			consultaFiltrada = sortingInfo.ColumnName switch
			{
				"id" => consultaFiltrada.SortBy(sortingInfo.Direction, e => e.Id),
				"color" => consultaFiltrada.SortBy(sortingInfo.Direction, e => e.Nombre),
