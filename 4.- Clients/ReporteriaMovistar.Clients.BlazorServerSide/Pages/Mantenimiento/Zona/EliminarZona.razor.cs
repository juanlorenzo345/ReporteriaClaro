...
{
	public partial class EliminarZona
	{
		#region Properties and Indexers
		
		[Parameter]
		public ViewZonaModel Modelo
		{
			get;
			set;
		}
		...
		private async Task EliminarAsync()
		{
			try
			{
				Result resultado = await this.zonaService.EliminarZonaAsync(
				...
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al eliminar la zona '{this.Modelo.Zona}' (ID {this.Modelo.Id}).", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"La zona '{this.Modelo.Zona}' se ha eliminado correctamente (ID {this.Modelo.Id}).", Severity.Success);
				this.MudDialog.Close(DialogResult.Ok(Modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al eliminar la zona '{this.Modelo.Zona}' (ID {this.Modelo.Id}).");
