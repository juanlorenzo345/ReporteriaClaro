...
// Info archivo:
//     Nombre: EditarZona.razor.cs
//     Fecha creación: 2021/11/02 at 05:21 PM
//     Fecha modificación: 2021/11/02 at 05:21 PM
// 
...
namespace ReporteriaMovistar.Clients.BlazorServerSide.Pages.Mantenimiento.Zona
{
	public partial class EditarZona
	{
		private MudForm formulario;

		private UpdateZonaModel modelo = new UpdateZonaModel();
		...
		{
			Result<ZonaEntityDto> resultadoZona = await this.zonaService.ObtenerZonaPorIdAsync(Id);
			
			if (resultadoZona.Type != ResultType.Succeeded)
			{
				this.snackbar.Add($"Se produjo un error al cargar los datos de la zona. {string.Join("\r\n", resultadoZona.Errors)}", Severity.Error);
				MudDialog.Cancel();
				return;
			}

			ZonaEntityDto dto = resultadoZona.Data;
		...
		public async Task GuardarAsync()
		{
			try
			{
				...
				Result resultado = await this.zonaService.ModificarZonaAsync(modelo);

				if (resultado.Type != ResultType.Succeeded)
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al modificar la zona '{this.modelo.Nombre}' (ID {this.Id}).", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"La zona '{this.modelo.Nombre}' se ha modificado correctamente (ID {this.Id}).", Severity.Success);
				MudDialog.Close(DialogResult.Ok(this.modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al modificar la zona '{this.modelo.Nombre}' (ID {this.Id}).");
