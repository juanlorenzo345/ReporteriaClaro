...
// Info archivo:
//     Nombre: CrearZona.razor.cs
//     Fecha creación: 2021/11/02 at 08:57 AM
//     Fecha modificación: 2021/11/02 at 08:57 AM
// 
...
{
	public partial class CrearZona
	{
		private MudForm formulario;

		private NewZonaModel modelo = new NewZonaModel();
		...
		public async Task GuardarAsync()
		{
			try
			{
				...
				Result resultado = await this.zonaService.CrearZonaAsync(modelo);

				if (resultado.Type != ResultType.Succeeded)
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al crear la zona '{this.modelo.Nombre}'.", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"La zona '{this.modelo.Nombre}' se ha creado correctamente.", Severity.Success);
				MudDialog.Close(DialogResult.Ok(this.modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al crear la zona {this.modelo.Nombre}.", Severity.Error);
