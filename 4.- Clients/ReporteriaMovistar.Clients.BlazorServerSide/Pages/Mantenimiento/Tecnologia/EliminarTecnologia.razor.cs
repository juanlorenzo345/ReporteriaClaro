...
// Info archivo:
//     Nombre: EliminarTecnologia.razor.cs
//     Fecha creación: 2021/11/03 at 03:57 PM
//     Fecha modificación: 2021/11/03 at 03:57 PM
// 
...
namespace ReporteriaMovistar.Clients.BlazorServerSide.Pages.Mantenimiento.Tecnologia
{
	public partial class EliminarTecnologia
	{
		#region Properties and Indexers

		[Parameter]
		public ViewTecnologiaEquipoModel Modelo
		{
			get;
			set;
		}
		...
		private async Task EliminarAsync()
		{
			try
			{
				Result resultado = await this.tecnologiaEquipoService.EliminarTecnologiaAsync(
				...
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al eliminar la tecnología '{this.Modelo.Tecnologia}' (ID {this.Modelo.Id}).", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"La tecnología '{this.Modelo.Tecnologia}' se ha eliminado correctamente (ID {this.Modelo.Id}).", Severity.Success);
				this.MudDialog.Close(DialogResult.Ok(Modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al eliminar la tecnología '{this.Modelo.Tecnologia}' (ID {this.Modelo.Id}).");
