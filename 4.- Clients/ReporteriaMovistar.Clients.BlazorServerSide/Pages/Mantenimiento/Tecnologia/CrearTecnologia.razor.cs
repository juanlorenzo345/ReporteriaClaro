...
// Info archivo:
//     Nombre: CrearTecnologia.razor.cs
//     Fecha creación: 2021/11/03 at 03:55 PM
...
{
	public partial class CrearTecnologia
	{
		private MudForm formulario;

		private NewTecnologiaEquipoModel modelo = new NewTecnologiaEquipoModel();
		...
		public async Task GuardarAsync()
		{
			try
			{
				...
				Result resultado = await this.tecnologiaEquipoService.CrearTecnologiaAsync(modelo);

				if (resultado.Type != ResultType.Succeeded)
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al crear la tecnología '{this.modelo.Nombre}'.", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"La tecnología '{this.modelo.Nombre}' se ha creado correctamente.", Severity.Success);
				MudDialog.Close(DialogResult.Ok(this.modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al crear la tecnología {this.modelo.Nombre}.", Severity.Error);
