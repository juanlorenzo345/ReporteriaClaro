...
// Info archivo:
//     Nombre: EliminarOperario.razor.cs
//     Fecha creación: 2021/11/17 at 11:29 AM
//     Fecha modificación: 2021/11/17 at 11:30 AM
// 
...
{
	public partial class EliminarOperario
	{
		#region Properties and Indexers

		[Parameter]
		public ViewOperarioModel Modelo
		{
			get;
			set;
		}
		...
		private async Task EliminarAsync()
		{
			try
			{
				Result resultado = await this.operarioService.EliminarOperarioAsync(
				...
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al eliminar el operario '{this.Modelo.Operario}' (ID {this.Modelo.Id}).", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"El operario '{this.Modelo.Operario}' se ha eliminado correctamente (ID {this.Modelo.Id}).", Severity.Success);
				this.MudDialog.Close(DialogResult.Ok(Modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al eliminar el operario '{this.Modelo.Operario}' (ID {this.Modelo.Id}).", Severity.Error);
