...
// Info archivo:
//     Nombre: EliminarConfiguracion.razor.cs
...
namespace ReporteriaMovistar.Clients.BlazorServerSide.Pages.Mantenimiento.Configuracion
{
	public partial class EliminarConfiguracion
	{
		#region Properties and Indexers

		[Parameter]
		public ViewConfiguracionEquipoModel Modelo
		{
			get;
			set;
		}
		...
		private async Task EliminarAsync()
		{
			try
			{
				Result resultado = await this.configuracionEquipoService.EliminarConfiguracionAsync(
				...
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al eliminar la configuración '{this.Modelo.Configuracion}' (ID {this.Modelo.Id}).", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"La configuración '{this.Modelo.Configuracion}' se ha eliminado correctamente (ID {this.Modelo.Id}).", Severity.Success);
				this.MudDialog.Close(DialogResult.Ok(Modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al eliminar la configuración '{this.Modelo.Configuracion}' (ID {this.Modelo.Id}).");
