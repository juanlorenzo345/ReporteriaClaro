...
// Info archivo:
//     Nombre: EliminarPronostico.razor.cs
//     Fecha creación: 2021/11/04 at 09:30 AM
//     Fecha modificación: 2021/11/04 at 09:30 AM
// 
...
{
	public partial class EliminarPronostico
	{
		#region Properties and Indexers

		[Parameter]
		public ViewPronosticoModel Modelo
		{
			get;
			set;
		}
		...
		private async Task EliminarAsync()
		{
			try
			{
				Result resultado = await this.pronosticoService.EliminarPronosticoAsync(
				...
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al eliminar el pronóstico de '{this.Modelo.Tecnologia}' para el período {this.Modelo.Periodo:MMMM yyyy} (ID {this.Modelo.Id}).", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"El pronóstico de '{this.Modelo.Tecnologia}' para el período {this.Modelo.Periodo:MMMM yyyy} se ha eliminado correctamente (ID {this.Modelo.Id}).", Severity.Success);
				this.MudDialog.Close(DialogResult.Ok(Modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al eliminar el pronóstico de '{this.Modelo.Tecnologia}' para el período {this.Modelo.Periodo:MMMM yyyy} (ID {this.Modelo.Id}).");
