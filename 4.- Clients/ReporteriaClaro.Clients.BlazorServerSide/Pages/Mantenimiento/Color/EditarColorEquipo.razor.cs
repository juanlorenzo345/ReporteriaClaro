...
{
    public partial class EditarColorEquipo
    {
        private MudForm formulario;

        private UpdateColorEquipoModel modelo = new UpdateColorEquipoModel();
        ...
        {
			Result<EquipoColorEntityDto> resultadoColor = await this.colorEquipoService.ObtenerColorPorIdAsync(Id);

			if(resultadoColor.Type != ResultType.Succeeded)
            {
				this.snackbar.Add($"Se produjo un error al cargar los datos del color. {string.Join("\r\n", resultadoColor.Errors)}", Severity.Error);
				MudDialog.Cancel();
				return;
            }

			EquipoColorEntityDto dto = resultadoColor.Data;
		...
		public async Task GuardarAsync()
		{
			try
			{
				...
				Result resultado = await this.colorEquipoService.ModificarColorAsync(modelo);

				if (resultado.Type != ResultType.Succeeded)
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al modificar el color '{this.modelo.Nombre}' (ID {this.Id}).", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"El color '{this.modelo.Nombre}' se ha modificado correctamente (ID {this.Id}).", Severity.Success);
				MudDialog.Close(DialogResult.Ok(this.modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al modificar el color {this.modelo.Nombre} (ID {this.Id}).", Severity.Error);
