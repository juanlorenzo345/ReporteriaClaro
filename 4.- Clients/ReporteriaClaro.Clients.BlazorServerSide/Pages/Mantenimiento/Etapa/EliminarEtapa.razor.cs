...
// Info archivo:
//     Nombre: EliminarEtapa.razor.cs
//     Fecha creación: 2021/11/04 at 04:00 PM
//     Fecha modificación: 2021/11/04 at 04:00 PM
// 
...
namespace ReporteriaClaro.Clients.BlazorServerSide.Pages.Mantenimiento.Etapa
{
	public partial class EliminarEtapa
	{
		#region Properties and Indexers

		[Parameter]
		public ViewEtapaModel Modelo
		{
			get;
			set;
		}
		...
		private async Task EliminarAsync()
		{
			try
			{
				Result resultado = await this.etapaService.EliminarEtapaAsync(
				...
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al eliminar la etapa '{this.Modelo.Etapa}' (ID {this.Modelo.Id}).", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"La etapa '{this.Modelo.Etapa}' se ha eliminado correctamente (ID {this.Modelo.Id}).", Severity.Success);
				this.MudDialog.Close(DialogResult.Ok(Modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al eliminar la etapa '{this.Modelo.Etapa}' (ID {this.Modelo.Id}).");
