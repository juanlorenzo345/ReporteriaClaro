...
{
    public partial class CrearMarcaEquipo
    {
        private MudForm formulario;

        private NewMarcaEquipoModel modelo = new NewMarcaEquipoModel();
		...
		public async Task GuardarAsync()
		{
			try
			{
				...
				Result resultado = await this.marcaEquipoService.CrearMarcaAsync(modelo);

				if (resultado.Type != ResultType.Succeeded)
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al crear la marca '{this.modelo.Nombre}'.", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"La marca '{this.modelo.Nombre}' se ha creado correctamente.", Severity.Success);
				MudDialog.Close(DialogResult.Ok(this.modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al crear la marca '{this.modelo.Nombre}'.", Severity.Error);
