...
// Info archivo:
//     Nombre: EliminarSecuenciaEtapa.razor.cs
//     Fecha creación: 2021/12/06 at 12:16 PM
...
{
	public partial class EliminarSecuenciaEtapa
	{
		#region Properties and Indexers

		[Parameter]
		public ViewSecuenciaEtapaModel Modelo
		{
			get;
			set;
		}
		...
		private async Task EliminarAsync()
		{
			try
			{
				Result resultado = await this.secuenciaEtapaService.EliminarSecuenciaAsync(
				...
				{
					string mensajeError = string.Join("\r\n", $"Se produjo un error al eliminar la secuencia '{this.Modelo.EtapaOrigen}' a '{this.Modelo.EtapaDestino}' (ID {this.Modelo.Id}).", string.Join("\r\n", resultado.Errors));
					this.snackbar.Add(mensajeError, Severity.Error);
					return;
				}

				this.snackbar.Add($"La secuencia '{this.Modelo.EtapaOrigen}' a '{this.Modelo.EtapaDestino}' se ha eliminado correctamente (ID {this.Modelo.Id}).", Severity.Success);
				this.MudDialog.Close(DialogResult.Ok(Modelo));
			}
			catch (Exception excepcion)
			{
				this.snackbar.Add($"Se produjo un error al eliminar la secuencia '{this.Modelo.EtapaOrigen}' a '{this.Modelo.EtapaDestino}' (ID {this.Modelo.Id}).", Severity.Error);
